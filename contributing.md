...
## 行为与兼容性
- **状态归一**：当前处理结果实际返回 `created`、`updated`、`skipped`、`AUTH_FAIL`、`BAD_INPUT`、`ERROR`，同时 `createDDNSResponse` 也会识别 `SUCCESS` 与 `NO_CHANGE` 并映射到 DynDNS/EasyDNS 规范（如 `good <ip>`、`nochg <ip>`、`badauth`、`911` 等）。
